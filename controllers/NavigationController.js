...
function(lang, declare, dom, domStyle, has, win, config, domClass, domAttr, domConstruct, Controller){
	...
	return declare("dojox/app/tests/mediaQuery3ColumnApp/controllers/NavigationController", Controller, {
		...
		constructor: function(app){
			...
			this.events = {
				"requestsApp/list": this._requestsList,
				"requestsApp/details": this._requestItemDetails,
				"requestsApp/search": this._requestListSearch
			//	"requestsApp/MainOption3": this.MainOption3,
			//	"requestsApp/LastOption1": this.LastOption1,
			//	"requestsApp/LastOption2": this.LastOption2,
			//	"requestsApp/LastOption3": this.LastOption3,
			//	"requestsApp/BackFromMain": this.BackFromMain,
			//	"requestsApp/BackFromTest": this.BackFromTest,
			//	"requestsApp/BackFromLast": this.BackFromLast
			};
		...
		_requestItemDetails: function(e){
		//	var params = {"mainCenter":{'mainSel':"MainOption1","tparam1":"tValue1"}};
			console.log("in NavigationController _requestItemDetails called.");
		...
		_requestListSearch: function(e){
		//	var params = {"mainCenter2":{'mainSel':"MainOption2"}};
			console.log("in NavigationController _requestListSearch called.");
			if(this.isSmall()){
				if(this.app.children.requestsApp_requestList){
					this.app.children.requestsApp_requestList.constraint = "left";
					domClass.remove(this.app.children.requestsApp_requestList.domNode, "center");
		...
		//	this.doTransition(e, "navLeft","mainCenter2","lastRight", params, false);
			var transOpts = {
							title: "requestListSearch",
							target: "requestListSearch",
							url: "#requestListSearch",
						//	reverse: back,
						//	"transition": "none",
							params:e.params
						};
	...
	*/
		...
		onResize: function(){
			...
			has.add("phone", this.isSmall(), 0, 1); // set has for phone

			if(this.lastSize == "small"){ // going from small to large need to
				if(this.app.children.requestsApp_requestList){
					domStyle.set(this.app.children.requestsApp_requestList.domNode, "display", "");
					this.app.children.requestsApp_requestList.constraint = "left";
					domClass.remove(this.app.children.requestsApp_requestList.domNode, "center");
					domClass.add(this.app.children.requestsApp_requestList.domNode, "left");
				}
